...
import threading
import pyMeow as pm
from keyauth import api  # Asegúrate de tener instalado el módulo keyauth-python-sdk

# Configuración de KeyAuth
keyauthapp = api(
    name="AutoHotkeyTest",  # App name
    ownerid="7qKDWi4hyd",   # Account ID
    version="1.0",          # Application version
    hash_to_check="PLACEHOLDER_HASH"  # Este hash se obtiene del método getchecksum()
)
...
def authenticate_user():
    ...
    try:
        keyauthapp.login(usuario, contrasena)
        print("Autenticación exitosa. Bienvenido.")
    except Exception as e:
        print(f"Error de autenticación: {e}")
        exit()

# Llama a la función de autenticación antes de continuar
authenticate_user()

...
window_ids = {'Priest': None, 'Soul Linker': None, 'Paladin': None, 'Creator': None, 'Professor': None}
window_pids = {'Priest': None, 'Soul Linker': None, 'Paladin': None, 'Creator': None, 'Professor': None}
...
friend_mouse_coordinates = {
    ...
    'Professor': {'x': None, 'y': None}
}

# Variable de control para el bucle Start/Stop
is_running = {'Priest': False, 'Soul Linker': False, 'Paladin': False, 'Creator': False, 'Professor': False}
                ...
                if module_base is not None:
                    ...
                    if handle:
                        try:
                            ...
                            # Validar y convertir a enteros
                            if None in (skill1_id_str, skill1_lvl_str, skill2_id_str, skill2_lvl_str):
                                print("Error: Uno o más campos de habilidad están vacíos en Creator.")
                                continue
                            ...
                            # SkillID específico para Creator
                            base_address_skillid = 0x00A43F44
                            offset_skillid = 0x3FC
                            offset_skilltype = 0x3F8
                            offset_skilllvl = 0x404
                            ...
                            print(f"Error: Asegúrate de ingresar números válidos para los IDs y niveles de habilidades en Creator.")
                        except Exception as e:
                            print(f"Error al escribir los valores para Creator: {e}")
                        finally:
                            ctypes.windll.kernel32.CloseHandle(handle)
                    else:
                        print(f"No se pudo abrir el proceso correctamente para Creator.")
                else:
                    print(f"No se pudo obtener la base del módulo correctamente para Creator.")
            else:
                print(f"No se ha seleccionado ninguna ventana para Creator.")
        else:
            print(f"No se ha seleccionado ninguna ventana para Creator.")
        time.sleep(1)

def write_skillid_skilltype_and_byte_values_professor():
    while is_running['Professor']:
        if window_pids['Professor']:
            pid = window_pids['Professor']
            game_process_name = get_process_name(pid)
            if game_process_name:
                module_name = game_process_name  # Ajusta si el nombre del módulo es diferente
                module_base = get_module_base_ctypes(pid, module_name)
                if module_base is not None:
                    ...
                    if handle:
                        try:
                            # Leer Skill 1 ID y Level desde la GUI
                            skill1_id_str = dpg.get_value("Professor_skill1_id")
                            skill1_lvl_str = dpg.get_value("Professor_skill1_lvl")
                            skill2_id_str = dpg.get_value("Professor_skill2_id")
                            skill2_lvl_str = dpg.get_value("Professor_skill2_lvl")

                            if DEBUG:
                                print(f"[Professor] Skill1 ID: {skill1_id_str}, Skill1 Level: {skill1_lvl_str}")
                                print(f"[Professor] Skill2 ID: {skill2_id_str}, Skill2 Level: {skill2_lvl_str}")

                            # Validar y convertir a enteros
                            if None in (skill1_id_str, skill1_lvl_str, skill2_id_str, skill2_lvl_str):
                                print("Error: Uno o más campos de habilidad están vacíos en Professor.")
                                continue
                            ...
                            print(f"Error: Asegúrate de ingresar números válidos para los IDs y niveles de habilidades en Professor.")
                        except Exception as e:
                            print(f"Error al escribir los valores para Professor: {e}")
                        finally:
                            ctypes.windll.kernel32.CloseHandle(handle)
                    else:
                        print(f"No se pudo abrir el proceso correctamente para Professor.")
                else:
                    print(f"No se pudo obtener la base del módulo correctamente para Professor.")
            else:
                print(f"No se ha seleccionado ninguna ventana para Professor.")
        else:
            print(f"No se ha seleccionado ninguna ventana para Professor.")
        time.sleep(1)
...
def toggle_start_stop(sender, app_data, user_data):
    ...
    if not is_running[window_key]:
        ...
        if window_key == 'Priest':
            ...
            threading.Thread(target=write_skillid_skilltype_and_byte_values_creator, daemon=True).start()
        elif window_key == 'Professor':
            threading.Thread(target=write_skillid_skilltype_and_byte_values_professor, daemon=True).start()
    else:
...
with dpg.window(label="Configuración de Autobuff", tag="primary_window", width=580, height=1080):
    ...
    create_section(
        title="Professor Config",
        key="Professor",
        default_skill1_id=402,
        default_skill1_lvl=5,
        default_skill2_id=402,
        default_skill2_lvl=5
    )

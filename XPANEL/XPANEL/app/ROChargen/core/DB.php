...
final class DB
{
	...
	static public function get_shadow_factor($id) {
		if( empty(self::$shadow) )
			self::$shadow = include_once(base_path('app/ROChargen/db/shadow.php'));
	...
	static public function get_npc_path($id)
	{
		// Load only if used
		if( empty(self::$mobs) ) {
			self::$mobs    = include_once(base_path('app/ROChargen/db/mobs.php'));
		}
	...
	static public function get_pet_accessory($id)
	{
		// Load only if used
		if( empty(self::$pets) ) {
			self::$pets    = include_once(base_path('app/ROChargen/db/pets.php'));
		}
	...
	{		
		...
		// Load only if used
		if( empty(self::$body) ) {
			self::$body    = include_once(base_path('app/ROChargen/db/body.php'));
		}
	...
	static public function get_body_pal_path($id,$sex,$pal)
	{
		...
		if( empty(self::$pals) ) {
			self::$pals    = include_once(base_path('app/ROChargen/db/pals.php'));
		}
	...
	static public function get_hat_path($id,$sex)
	{
		...
		if( empty(self::$hats) ) {
			self::$hats = include_once(base_path('app/ROChargen/db/hats.php'));
		}
	...
	static public function get_weapon_path($job_id, $sex, $weapon_id )
	{
		...
		if( empty(self::$weapon) ) self::$weapon  = include_once(base_path('app/ROChargen/db/weapon.php'));
		if( empty(self::$body) )   self::$body    = include_once(base_path('app/ROChargen/db/body.php'));
	...
	static public function get_shield_path($job_id, $sex, $shield_id )
	{
		...
		if( empty(self::$shield))  self::$shield  = include_once(base_path('app/ROChargen/db/shield.php'));
		if( empty(self::$body) )   self::$body    = include_once(base_path('app/ROChargen/db/body.php'));
	...
	static public function get_robe_path( $job_id, $sex, $robe_id )
	{
		...
		if( empty(self::$robes['list']) ) {
			self::$robes['list']    = include_once(base_path('app/ROChargen/db/robe.php'));
			self::$robes['inherit'] = include_once(base_path('app/ROChargen/db/inherit.robe.php'));
	...
	static public function robe_ontop( $job_id, $_sex, $robe_id, $action, $animation )
	{
		...
		if( empty(self::$robes[$size][$sex]) ) {
			self::$robes[$size][$sex] = include_once(base_path('app/ROChargen/db/'.$size.'_'.$_sex.'.robe.php'));
		}

...
const blacksmiths = usePage().props.blacksmiths;
const url = ref({});
const rank_attributes = ref(["Player", "Guild", "Level", "Online", "Points"]);
</script>
...
<template>
    <ranking-panel title="Deaths Counter">
        <ranking-table :rank_attributes="rank_attributes">
            <template #row>
                <tr
                    class="spaceUnder hover:bg-gray-300/40 dark:hover:bg-white/20"
                    v-for="(blacksmith, index) in blacksmiths.data"
                    :key="index"
                >
                    ...
                    <td class="px-5 py-3">
                        <div class="flex items-center justify-center">
                            <img
                                :src="`/rochar/character-head/${blacksmith.name}`"
                                alt=""
                            />
                            <a href="">
                                <span class="text-[#3b87f6]">
                                    {{ blacksmith.char_name }}
                                </span>
                                ({{ get_job_name(blacksmith.char_class) }})
                            </a>
                        </div>
                    </td>
                    ...
                    <td class="px-5 py-3">
                        <div class="flex items-center justify-center">
                            <div class="flex items-center">
                                <img
                                    v-if="blacksmith.emblem != 'X'"
                                    class="mr-2"
                                    :src="`/${blacksmith.emblem} `"
                                    alt=""
                                />
                                {{ blacksmith.guild_name ?? "No guild ❌" }}
                            </div>
                        </div>
                    </td>
                    <td class="px-5 py-3">
                        {{ blacksmith.base_level }} / {{ blacksmith.job_level }}
                    </td>
                    <td class="px-5 py-3">
                        <span
                            v-if="blacksmith.online"
                    ...
                    <td class="px-5 py-3">
                        {{ getFormatNumber(blacksmith.fame) }}
                    </td>
                </tr>
            </template>
        </ranking-table>
        <pagination class="p-3" :links="blacksmiths.links" />
    </ranking-panel>
</template>
